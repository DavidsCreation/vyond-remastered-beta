...
function adjust_studio() {
    if (!resize_studio) {
        return
    }
    var b = 0;
    b += (show_importer ? 260 : 0);
    var d = Math.max(jQuery(window).width(), STUDIO_MIN_WIDTH + b);
    jQuery("#studio_container").width(d);
    jQuery("#studio_holder").width(d - b).toggleClass("offset", show_importer);
    if (jQuery("#studio_container .ga-importer").length > 0) {
        ...
        jQuery(".ga-importer-content").height(c)
    }
    console.log(jQuery(window).height());
    console.log(jQuery("div.navbar").length);
    var a = jQuery(window).height() - (jQuery(".navbar").length > 0 ? 50 : 0);
    a = Math.max(a, STUDIO_MIN_HEIGHT);
    console.log(STUDIO_MIN_HEIGHT);
    console.log(a);
...
function showCCBrowser(b) {
    ...
    ga("send", "pageview", "/pageTracker/ccbrowser/open");
...
function hideCCBrowser() {
    ...
    ga("send", "pageview", "/pageTracker/ccbrowser/close");
    restoreStudio();
    return false
}
var interactiveTutorial = {
    cookieName: "studio_interactive_tutorial",
    neverDisplay: function(a) {
        if (a === undefined) {
            return !this.needToShow()
        }
        var b = (a) ? "skip" : "show";
        SetCookie(this.cookieName, b, 500, "/")
    },
    needToShow: function() {
        if (this.forceShow) {
            return true
        }
        var a = getCookie(this.cookieName);
        return (a === "skip") ? false : true
    }
};
var studioApi = function(d) {
    ...
    var b = e || {
        ...
        importerUploadComplete: function(f, h, g) {
            b.getStudio().importerUploadComplete(f, h, g);
            return b
        },
