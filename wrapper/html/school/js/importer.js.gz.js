...
Importer.prototype.initialize = function() {
    var a = this;
    ...
    this.$el.find("i.i-help").hover(function() {
        $(this).next(".tooltip").fadeIn()
    }, function() {
        $(this).next(".tooltip").fadeOut()
    });
    if (this.studio) {
        this.studio.bind("userAssetReady", $.proxy(this.onAssetReady, this)).bind("userAssetDelete", $.proxy(this.onAssetDelete, this))
    }
};
Importer.prototype.show = function() {
    this.$el.addClass("show")
};
Importer.prototype.hide = function() {
    this.$el.removeClass("show")
};
Importer.prototype.clearList = function() {
    this.$list.find(".imported, .canceled, .invalid").remove();
    this.updateResults()
};
Importer.prototype.updateResults = function() {
    this.importedCount = this.$list.children(".imported").length;
    var a = this.$list.children(".processing").length,
        b = this.$list.children().length,
    ...
    if (this.importedCount == 0) {
        ...
        if (this.importedCount == 1) {
            $(".ga-importer-notice-count").text(GT.strargs("%1 file has been added to Your Library.", [1]))
        } else {
            $(".ga-importer-notice-count").text(GT.strargs("%1 files have been added to Your Library.", [this.importedCount]))
        }
...
Importer.prototype.checkList = function() {
...
Importer.prototype.addFiles = function(a) {
    if (a.length == 0) {
        return
    }
...
Importer.prototype.uploadFile = function(c) {
    var a = this;
    if (this.studio) {
...
Importer.prototype.fileUploaded = function(b, a) {
    if (a.suc) {
        b.asset.id = a.id;
        ...
        if (a.asset_type == "video") {
            ...
            if (this.studio) {
                b.studio = this.studio;
                this.studio.importerUploadComplete(b.asset.type, b.asset.id, b.asset.data)
            }
        }
    } else {
        b.status("invalid");
...
Importer.prototype.findImportedByFileId = function(b) {
    var a = false;
...
Importer.prototype.onAssetReady = function(b, a) {
    var c = this.findImportedByFileId(a.file);
    if (c) {
        c.data("importerFile").setReady()
    }
};
Importer.prototype.onAssetDelete = function(b, a) {
    var c = this.findImportedByFileId(a.file);
    if (c) {
        c.data("importerFile").setDeleted()
    }
};
...
ImporterFile.prototype.initialize = function() {
    ...
    this.$el.on("click", '[data-action="add-to-scene"]', function(c) {
        c.preventDefault();
        b.addToScene()
...
ImporterFile.prototype.fileTypeCheck = function() {
    if ($.inArray(this.file.type, this.options.restricted_mime) >= 0) {
        throw "Your current plan does not support this file type"
    }
    if ($.inArray(this.file.type, this.options.accept_mime) < 0) {
...
ImporterFile.prototype.setIcon = function(d) {
    ...
    var c = ["prop", "image", "flash", "bg", "sound", "video", "invalid", "loading"];
    ...
    $.post("/ajax/getAssetVideoStatus/" + this.asset.id, function(b) {
        if (b.suc) {
            if (b.status == "completed") {
                ...
                a.setIcon("video");
                if (a.studio) {
                    a.studio.importerUploadComplete(a.asset.type, a.asset.id, a.asset.data)
                }
...
ImporterFile.defaults.options = {
    accept_mime: ["image/png", "image/jpeg", "image/gif", "image/bmp", "audio/mpeg", "audio/wav", "audio/x-wav", "audio/vnd.wave", "audio/wave", "audio/mp3", "audio/mp4", "audio/ogg", "audio/vorbis", "audio/aac", "audio/m4a", "audio/x-m4a", "application/x-shockwave-flash", "video/mp4", "video/x-flv", "video/x-ms-wmv"],
    restricted_mime: [],
