...
function parseResponse(B) {
    ...
    var C = B.indexOf("{");
    var A = B.indexOf("}");
    responseArray.json = B.substring(C, A + 1).evalJSON();
...
function clearFeedback() {
    var A = document.getElementById("feedback_block");
...
function displayFeedback(E) {
    ...
    var B = E.substring(1);
    if (view_name == "school") {
        var D = document.getElementById("growlfeedback");
        if (D) {
            D.innerHTML = B
        }
        var A = document.getElementById("growlfeedback_block");
        if (C == "0") {
...
function tutpiper(C) {
    ...
    for (var B = 0; B < C.value.items.length; B++) {
        ...
        A += GT.gettext(C.value.items[B].title) + "</a></div><br>";
        if (C.value.items[B].description) {
            A += '<div class="gotipsblockdesc">' + GT.gettext(C.value.items[B].description) + "</div>"
        }
...
function getFeed(D, C, B) {
    ...
    if (B == "index") {
        A.src = "http://web.archive.org/web/20120209080457/http://pipes.yahoo.com/pipes/pipe.run?_id=a9d57eb2ac1388d7a0f9d50978bed7c2&_render=json&_callback=piper&length=" + C + "&feed=" + D
    } else {
        if (B == "htw") {
            A.src = "http://web.archive.org/web/20120209080457/http://pipes.yahoo.com/pipes/pipe.run?_id=2569724e3c819b5f0c52efd4e8fea03f&_render=json&_callback=tutpiper&length=" + C + "&feed=" + D
        }
    }
...
function showHTMLBox(G, F, D, B, E, A) {
    $("overlayHTMLBoxTitle").innerHTML = (G === "" ? "&nbsp;" : G);
    $("HTMLBoxMessage").innerHTML = D;
    ...
    var C = currPos();
    grayOutGlobal(true, B, E);
    $("overlayObjectGlobal").style.top = F + C[1] + "px"
}
...
function showConfirmBox(F, E, C, G, A, D) {
    $("overlayConfirmBoxTitle").innerHTML = (F === "" ? "&nbsp;" : F);
    $("overlayConfirmConfirmBtn").innerHTML = "<input type='button' onClick='javascript:" + G + ";' value='" + GT.gettext("Confirm") + "'>";
    ...
    var B = currPos();
    grayOut(true, "showConfirmBox", D);
    $("overlayObject").style.top = E + B[1] + "px"
}
...
function showInputBox(F, E, C, G, A, D) {
    $("overlayInputBoxTitle").innerHTML = (F === "" ? "&nbsp;" : F);
    $("overlayInputSaveBtn").innerHTML = "<input type='button' onClick='javascript:" + G + ";' value='" + GT.gettext("Save") + "'>";
...
function showProgressBox(G, E, F, B, J, C, H, A, I) {
    $("overlayProgressBoxTitle").innerHTML = (G === "" ? "&nbsp;" : G);
    $("progressTotal").innerHTML = F;
    $("progressCurr").innerHTML = "1";
    $("progressBoxMessage").innerHTML = J;
    $("overlayProgressClose").innerHTML = '<a href="javascript:' + A + '"></a>';
    $("overlayProgressBoxBtn").innerHTML = '<input id="overlayProgressBtn" type="button" value="' + C + '" onClick="javascript:' + H + '">';
    ...
    $("overlayObject").style.top = E + D[1] + "px"
}
...
function showLogin(C, A, D) {
    ...
    $("overlayObject2").style.margin = C + "px 0px 0px 0px";
    if (A && A.length > 0) {
...
function showBizSignup(C, A) {
    var B = currPos();
    C = B[1];
    $("overlayBizSignup").style.display = "block";
    $("overlayObject2").style.margin = C + "px 0px 0px 0px";
    if (A && A.length > 0) {
        document.bizsignupform.returnto.value = A
    }
...
function sendBizSignupComplete(A) {
    parseResponse(A);
    $("bizsignupform").enable();
    switch (A.charAt(A.indexOf("{") - 1)) {
        case "0":
...
function chooseLanguage(A) {
    new Ajax.Request(URL_CHOOSE_LANGUAGE, {
        method: "post",
        parameters: "lang=" + A + "&uri=" + window.location,
        onSuccess: function(C) {
            var B = C.responseText;
            chooseLanguageComplete(B)
        },
        onFailure: function() {
            displayFeedback("1" + GT.gettext("Error contacting the server"))
        }
...
function showUltimate(B) {
    var A = currPos();
    B = A[1];
    $("overlayUltimate").style.display = "block";
    $("overlayObject2").style.margin = B + "px 0px 0px 0px";
